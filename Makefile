...
# Use DEVNET=1 for local devnet (PostgreSQL + Tempo node)
...
test:
	@$(DEVNET_COMPOSE) up -d postgres tempo
	@echo "Waiting for PostgreSQL..."
	@until $(DEVNET_COMPOSE) exec -T postgres pg_isready -U ak47 -d postgres > /dev/null 2>&1; do sleep 1; done
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d postgres -c "CREATE DATABASE ak47" > /dev/null 2>&1 || true
...
define check_bench_data
	@TX_COUNT=$$($(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47 -tAc "SELECT COUNT(*) FROM txs" 2>/dev/null || echo "0"); \
...
bench-gen:
	...
	@START_TIME=$$(date +%s); \
	$(DEVNET_COMPOSE) up -d postgres; \
	echo "Waiting for PostgreSQL..."; \
	until $(DEVNET_COMPOSE) exec -T postgres pg_isready -U ak47 -d postgres > /dev/null 2>&1; do sleep 1; done; \
	$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d postgres -c "DROP DATABASE IF EXISTS ak47_test WITH (FORCE)" > /dev/null 2>&1 || true; \
	$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d postgres -c "CREATE DATABASE ak47_test" > /dev/null; \
	...
	$(DEVNET_COMPOSE) exec -T postgres pg_dump -U ak47 -Fc ak47_test > $(BENCH_ARTIFACT); \
	END_TIME=$$(date +%s); \
	...
	$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47_test -c "SELECT COUNT(*) as blocks FROM blocks; SELECT COUNT(*) as txs FROM txs; SELECT COUNT(*) as logs FROM logs;"

# Internal: restore from artifact (fast)
_bench_restore:
	@$(DEVNET_COMPOSE) up -d postgres
	@until $(DEVNET_COMPOSE) exec -T postgres pg_isready -U ak47 -d postgres > /dev/null 2>&1; do sleep 1; done
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d postgres -c "DROP DATABASE IF EXISTS ak47 WITH (FORCE)" > /dev/null 2>&1 || true
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d postgres -c "CREATE DATABASE ak47" > /dev/null
	@cat $(BENCH_ARTIFACT) | $(DEVNET_COMPOSE) exec -T postgres pg_restore -U ak47 -d ak47 --no-owner --no-acl 2>/dev/null || true
	@TX_COUNT=$$($(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47 -tAc "SELECT COUNT(*) FROM txs"); \
...
_bench_seed:
	...
	@SEED_TXS=$(BENCH_TXS) DATABASE_URL=postgres://ak47:ak47@localhost:5433/ak47 \
		cargo test --release --test seed_bench -- --ignored --nocapture
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47 -c "SELECT COUNT(*) as blocks FROM blocks; SELECT COUNT(*) as txs FROM txs; SELECT COUNT(*) as logs FROM logs;"
...
bench-vs-golden-axe:
	...
	@$(DEVNET_COMPOSE) up -d tempo postgres
	...
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -c "DROP DATABASE IF EXISTS ak47" > /dev/null 2>&1 || true
	@$(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -c "CREATE DATABASE ak47" > /dev/null
	...
	END=$$(date +%s); \
	AK47_TIME=$$((END - START)); \
	AK47_TXS=$$($(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47 -tAc "SELECT COUNT(*) FROM txs" 2>/dev/null || echo "0"); \
	AK47_BLOCKS=$$($(DEVNET_COMPOSE) exec -T postgres psql -U ak47 -d ak47 -tAc "SELECT COUNT(*) FROM blocks" 2>/dev/null || echo "0"); \
...
help:
	...
	@echo ""
	@echo "  DEVNET=1 make <cmd>    Use devnet compose (PostgreSQL + local Tempo)"
	@echo ""
